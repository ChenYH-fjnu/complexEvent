...
public class weatherSFA {
    ...
    static SFA<CharPred, Character> testPattern(UnaryCharIntervalSolver ba) {
        Collection<SFAMove<CharPred, Character>> transitionsA = new LinkedList<SFAMove<CharPred, Character>>();
        transitionsA.add(new SFAInputMove<CharPred, Character>(1, 2,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(1, 3,snow));


        transitionsA.add(new SFAInputMove<CharPred, Character>(2, 4,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(2, 5,snow)); //

        transitionsA.add(new SFAInputMove<CharPred, Character>(3, 4,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(3, 5,snow)); //

        transitionsA.add(new SFAInputMove<CharPred, Character>(4, 6,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(4, 7,snow)); //

        transitionsA.add(new SFAInputMove<CharPred, Character>(6, 8,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(6, 9,snow)); //

        transitionsA.add(new SFAInputMove<CharPred, Character>(7, 8,rain)); //
        transitionsA.add(new SFAInputMove<CharPred, Character>(7, 9,snow)); //
        try {
            return SFA.MkSFA(transitionsA, 1, Arrays.asList(8,9), ba);
        } catch (TimeoutException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) {
        ...
        SFA<CharPred, Character> sfaOld= weatherSFAForPattern(ba);
        System.out.println(sfaOld.getTransitions());
    }
