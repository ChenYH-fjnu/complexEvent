...
public class PSTree {
    ...
    public static double travelPSTree(PSTTreeNode<String> root,String memory,int predicatesIndex){
        ...
        if (memory.length()==0){
            ...
            boolean exist=false;
//            for (int i=memory.length()-1;i>=0;i--){ //3
            if (root.getChildren().size()>0) {
                for (PSTTreeNode<String> child : root.getChildren()) {//第一层
    ...
    public static PSTTree<String> createPSTree(Tree<String> tree,String[] predicates,int order,int firstConditionsherFre,double secondConditionSher) {
        ...
        while (!PSTTreeNodeQueue.isEmpty()) {
            ...
            String suffix = secondFloorNode.getStr();  //b
            int fre = 0;
...
//        int firstConditionsherFre = 1;
////        double secondConditionSher = 0.95;
        if (m > 1) {
            for (String childnode :  predicates) {
                ...
                for (int i = 0; i <  predicates.length; i++) {
                    double pro1 = childNode.getProbabilities(i);  // ?|aa
                    double pro2 = parantNode.getProbabilities(i); // ?|a
...
//        String[] weatherPredicates = {"a","b","c","d","e","f","s"};
//        String predictionDataPathDic = "E:\\陈二十\\Data\\archive\\dataProcessed\\dataForModeling\\biggestData/Saguache.txt";
//        suffixTree.Tree<String> tree = suffixTreeModel4Weather(predictionDataPathDic, 7 ); //这个函数与Pattern无关
//
//        int con1=0;
//        double con2=0.0;
//        PSTTree<String> PSTree = createPSTree(tree, weatherPredicates, 7,con1,con2 ); //建树的过程
//        long t1 = System.nanoTime();
//        travelPSTree2(PSTree,"ssssss",0);
//
//        long t2 = System.nanoTime();
//        travelPSTree(PSTree.getRoot(),"ssssss",0);
